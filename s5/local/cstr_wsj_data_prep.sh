...
cat $CORPUS/wsj1/wsj1/doc/indices/wsj0/train/tr_s_wv1.ndx \
  | $local/cstr_ndx2flist.pl $CORPUS | sort  \
  | grep -v wsj0/si_tr_s/401 > train_si84.flist

# This version for SI-284

# cat $CORPUS/wsj1/wsj1/doc/indices/si_tr_s2.ndx \
#   $CORPUS/wsj1/wsj1/doc/indices/wsj1/train/tr_s_wv12.ndx \
#   | $local/cstr_ndx2flist.pl  $CORPUS | sort \
#   | grep -v wsj1/si_tr_s/401 > train_si284.flist

# cat $CORPUS/wsj1/wsj1/doc/indices/si_tr_s2.ndx \
#   $CORPUS/wsj1/wsj1/doc/indices/wsj1/train/tr_s_wv1.ndx \
#   | $local/cstr_ndx2flist.pl  $CORPUS | sort > train_si284.flist
...
find $CORPUS/wsj1/wsj1/??_??_20 -print | grep -i ".wv1" | sort > dev_dt_20.flist
find $CORPUS/wsj1/wsj1/??_??_05 -print | grep -i ".wv1" | sort > dev_dt_05.flist


# Finding the transcript files:
find -L $CORPUS/wsj1 -iname '*.dot' > dot_files.flist
find -L $CORPUS/wsj0 -iname '*.dot' >> dot_files.flist
...
for x in  train_si284 test_eval93 test_dev93 test_eval93_5k test_dev93_5k dev_dt_05 dev_dt_20 ; do
  $local/flist2scp.pl $x.flist | sort > ${x}_sph.scp
  cat ${x}_sph.scp | awk '{print $1}' \
    | $local/find_transcripts.pl dot_files.flist > $x.trans1
done
...
cp $CORPUS/wsj1/wsj1/doc/lng_modl/vocab/wfl_64.lst $lmdir
...
cp $CORPUS/wsj1/wsj1/doc/lng_modl/base_lm/bcb20onp.z $lmdir/lm_bg.arpa.gz || exit 1;
chmod u+w $lmdir/lm_bg.arpa.gz

# trigram would be:
cat $CORPUS/wsj1/wsj1/doc/lng_modl/base_lm/tcb20onp.z | \
...
cp $CORPUS/wsj1/wsj1/doc/lng_modl/base_lm/bcb05onp.z  $lmdir/lm_bg_5k.arpa.gz || exit 1;
chmod u+w $lmdir/lm_bg_5k.arpa.gz

# trigram would be: !only closed vocabulary here!
cp $CORPUS/wsj1/wsj1/doc/lng_modl/base_lm/tcb05cnp.z $lmdir/lm_tg_5k.arpa.gz || exit 1;
...
cat $CORPUS/wsj0/wsj0/doc/spkrinfo.txt \
    $CORPUS/wsj1/wsj1/doc/evl_spok/spkrinfo.txt \
    $CORPUS/wsj1/wsj1/doc/dev_spok/spkrinfo.txt \
    $CORPUS/wsj1/wsj1/doc/train/spkrinfo.txt \
